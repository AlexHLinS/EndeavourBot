...
def oggToWav(ogg_file_name):
    ...
    try:
        ...
        print(
            f'[oggToWav] load audio segment from {ogg_file_name} ...', end='')

        phrase = AudioSegment.from_ogg(ogg_file_name)
        print('Done!')
        print(
            f'[oggToWav] export from {ogg_file_name} to {result_file_name} ...', end='')
...
def speechToText(wav_file_name, language):
    ...
    try:
        print(
            f'[speechToText] sending {audio.sample_width} to cloud ...', end='')
